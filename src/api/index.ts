...
import { GrowEvent, GrowEventDto, Profile, ProfileDto, Sponsor, UserRole } from "types";
...
export const getRegistrationsOfUser = async (user_id: string) => await supabaseClient
...
export const getSponsors = async () => await supabaseClient
...
export const getProfile = async (user_id: string) => await supabaseClient
...
export const updateProfile = async (userId: string, dto: ProfileDto) => await supabaseClient
...
export const createEvent = async (growEvent: Partial<GrowEvent>) => await supabaseClient
...
export const updateEvent = async (eventId: number, patch: Partial<GrowEvent>) => await supabaseClient
    .from<GrowEvent>('events')
    .update(patch)
    .match({ id: eventId });
